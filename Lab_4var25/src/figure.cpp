...
Square::~Square() {
    // No need to clear static arrays, but you can set them to default values if necessary
    for (int i = 0; i < 4; ++i) {
        x_kords[i] = 0.0;
        y_kords[i] = 0.0;
    }
...
Square &Square::operator=(Square &&other) {
    // Moving primitive types (no need to move arrays)
    mid_x = std::move(other.mid_x);
    mid_y = std::move(other.mid_y);
    surface = std::move(other.surface);
    
    // Deep copy for the arrays as we can't move static arrays
    for (int i = 0; i < 4; ++i) {
        x_kords[i] = other.x_kords[i];
        y_kords[i] = other.y_kords[i];
    }
...
Triangle::~Triangle() {
    // Similar to Square, no need to clear static arrays
    for (int i = 0; i < 3; ++i) {
        x_kords[i] = 0.0;
        y_kords[i] = 0.0;
    }
...
Triangle &Triangle::operator=(Triangle &&other) {
    ...
    // Moving arrays is not supported, so deep copy them
    for (int i = 0; i < 3; ++i) {
        x_kords[i] = other.x_kords[i];
        y_kords[i] = other.y_kords[i];
    }
...
Rectangle::~Rectangle() {
    // No need to clear static arrays
    for (int i = 0; i < 4; ++i) {
        x_kords[i] = 0.0;
        y_kords[i] = 0.0;
    }
